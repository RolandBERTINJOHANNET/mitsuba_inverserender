...
public:
    CustomTraversalCallback(bool chcol) { change_color = chcol; }
...
class BRDFTraversalCallback : public TraversalCallback {

private:
    bool change_color;

public:
    BRDFTraversalCallback(bool chcol) { change_color = chcol; }
    void put_parameter_impl(const std::string &name, void *ptr, uint32_t flags,
                            const std::type_info &type) {
        std::cout << "found param " << name << "which is differentiable "
...
void CustomTraversalCallback::put_object(const std::string &name, Object *obj,
                                         uint32_t flags) {
    if (name == "LeftWallBSDF") {
        std::cout << "traversing LeftWallBSDF : \n" << std::endl;
        BRDFTraversalCallback cb(change_color);
        (obj)->traverse(&cb);
        std::cout << std::endl;
    }
...
void render(Object *scene_, size_t sensor_i, fs::path filename) {
    ...
    /* for (int i = 0; i < film->bitmap()->size()[0]; i++) {
        for (int j = 0; j < film->bitmap()->size()[1]; j++) {
            std::cout
                << (int) (film->bitmap()
                          ->uint8_data())[i * film->bitmap()->size()[1] + j]
                << std::endl;
        }
    }*/
...
/*
    ...
    //show difference
    std::cout << "reference bitmap sizes : " << (int)referenceBitmap.size()[0] << ", " << (int) referenceBitmap.size()[1] << std::endl;
    for (int i = 0; i < newBitmap.size()[0]; i++) {
        for (int j = 0; j < newBitmap.size()[1]; j++) {
            std::cout << "printing a pixel" << std::endl;
            std::cout
                << (int) (referenceBitmap.uint8_data())[i * newBitmap.size()[1] + j]
                << std::endl;
            /*std::cout
                << (int) abs(
                       (newBitmap->uint8_data())[i * newBitmap->size()[1] + j] -
                       (referenceBitmap
